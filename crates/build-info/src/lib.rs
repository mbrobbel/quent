...
pub struct ModelInfo {
    ...
    /// Cargo package of the analyzer crate that provides a `QuentViewer` entry
    /// for this model (e.g. `"quent-simulator-analyzer"`). `None` when the model
    /// did not declare one; the analyzer's source git is the model's
    /// [`source`](Self::source).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub trait ModelSource {
    ...
    /// Git provenance of the crate defining the model.
    fn source() -> BuildInfo;
    /// Cargo package of the analyzer crate providing this model's `QuentViewer`
    /// entry, if the model declared one (`analyzer_package` in `model!`). It is
    /// assumed to share the model's [`source`](Self::source) git.
